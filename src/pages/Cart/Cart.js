...
export default function Cart() {
  const { products, cart,onSale,removeFromCart,increase,decrease } = useContext(AppContext);
  ...
  fullPrice=cart.reduce((prev,curr)=>{
    ...
    if(product.totalQuantity>20){
      return(
        prev+(curr.price-(curr.price*curr.discount)/10)*curr.quantityInCart
      )
    }
    else{
      return(
        prev+curr.price*curr.quantityInCart
      )
    }
  },0)
  ...
  return (
    ...
    <div style={{display:"flex",justifyContent:"start"}} ><h3 style={{margin:"20px 0 0 3vw",alignSelf:"start"}}>{cart.length} {cart.length ===1?"ITEM":"ITEMS"}</h3></div>
      ...
      <div style={{display:"flex",justifyContent:"start"}} ><h2>ORDER SUMMARY</h2></div>
        ...
        <input type='text' id='discount' disabled={promo} name='discount' value={discount} onKeyDown={handleDiscount} onChange={event => setDiscount(event.target.value)} placeholder={placeholder}></input><LoyaltyIcon className='loyalty'/>
        </div>
        ...
        <div style={{display:"flex",justifyContent:"space-between",marginTop:"15px"}}>
          ...
          <h4 style={{fontWeight:"800"}}>${promo?fullPrice-5:fullPrice}  </h4>
        </div>
        ...
        ):<div style={{width:"100vvw",height:"75vh",backgroundColor:"#F6F6F6"}}>
          <div className='img-div'>
            <img src='https://www.pngkey.com/png/detail/322-3224343_empty-cart-icon-png-empty-shopping-cart-icon.png' alt='/'></img>
            <h2 style={{position:"absolute",top:"73vh",left:"0",right:"0",margin:"auto"}}>CART IS EMPTY</h2>
        </div>
        </div>
      }
    </>
